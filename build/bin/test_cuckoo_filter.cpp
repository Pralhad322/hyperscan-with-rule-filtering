...
#include "cuckoo_based_algorithm/cuckoo_rule_processor.h"
...
int main() {
    ...
    // Initialize CuckooRuleDatabase
    CuckooRuleDatabase rule_db;
    
    // Add rules to database
    for (const auto& [rule_id, rule_info] : rules) {
        const auto& [string_ids, sid] = rule_info;
        rule_db.add_rule(rule_id, string_ids, sid);
    }
    
    rule_db.finalize_preprocessing();
    std::cout << "Cuckoo rule database initialized and preprocessed" << std::endl;
    
    // Test packet processing
    CuckooPacketMatchProcessor processor(rule_db);
    ...
    std::cout << "\n=== Cuckoo Filter Results ===" << std::endl;
